...
- CI benchmarks that are stored back in the repository and gate later commits on performance regressions.
...
CI emits structured benchmark JSON. Successful branch pushes commit the result back into:

#+begin_example
bench/results/latest.json
bench/results/history/<git-commit>.json
#+end_example

The next commit compares against the committed baseline. CI fails when the optimized path is materially slower and its relative advantage also degrades beyond the configured tolerance. Benchmark-result-only commits use =[skip ci]= and are ignored by the workflow trigger, so the write-back cannot loop forever.
