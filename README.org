...
- a durable symbolic-AI substrate with content-addressed facts, declared experts,
  safe conjunctive rules, bounded forward inference, variable queries, and
  proof provenance.
...
Long-running importers can atomically commit normalized documents, graph nodes,
graph edges, and a source watermark. Tek9 does not know the upstream schema;
the adapter supplies already-normalized records.
...
(apply-ingest-batch
 ...
 :edges graph-edges
 :graph-name "music"
 :watermark "last-media-file-id")
#+end_src

The generation fence rejects stale writers. The checkpoint advances only if
every document and graph mutation commits successfully; any error aborts the
entire LMDB transaction.

#+begin_src lisp
(fetch-ingest-checkpoint db "navidrome")
;; => (:SOURCE-ID "navidrome" :GENERATION 42
;;     :WATERMARK "last-media-file-id" :COMMITTED-AT ...)
#+end_src

Use this for canonical mirrors and resumable imports. Keep upstream-specific
SQLite/API/file parsing outside Tek9.
...
Tek9 includes a small, deterministic Datalog-style symbolic layer. It is an
embedded reasoning substrate, not an LLM and not an arbitrary code executor.
Facts, rule manifests, expert manifests, assertions, and derivation provenance
are durable Tek9 records. Rule bodies are data: Tek9 never =eval=s them and does
not expose a raw Prolog-goal escape hatch.

Ground facts are content-addressed by their normalized term:

#+begin_src lisp
(symbolic-assert-fact *db* '(parent "alice" "bob")
                      :source-ids '("import:family"))

(symbolic-assert-fact *db* '(parent "bob" "carol"))
#+end_src

Queries use symbols whose names begin with =?= as variables. Every query is
candidate-bounded and returns a second value when its bound made the answer
incomplete:

#+begin_src lisp
(symbolic-query *db* '(parent "alice" ?who))
;; => ((:FACT-ID ... :TERM ("parent" "alice" "bob")
;;      :BINDINGS ((?WHO . "bob"))))
;;    NIL
#+end_src

Experts are immutable capability manifests. Their input/output predicate
declarations are enforced when rules are registered:
...
Consequent variables must be bound by antecedents (range-restricted rules).
Predicates are always ground names. This makes rules portable to external
reasoners such as SWI-Prolog without making Tek9 itself accept executable Prolog
text.

Run one expert to a bounded fixed point:

#+begin_src lisp
(symbolic-run-expert *db* "kinship/1")
;; => (:ROUNDS 2 :DERIVED 1 :DERIVATIONS 1 :SATURATED T)

(symbolic-query *db* '(grandparent "alice" ?who))
#+end_src

Or infer and query in one operation:
...
Inference has explicit bounds for rules, rounds, candidate facts, binding
states, and newly derived facts. Exceeding a bound signals
=SYMBOLIC-INFERENCE-LIMIT= instead of returning an unsound partial proof.

Derived facts record the rule, expert, and evidence fact IDs that produced
them. Explicit assertions keep their source IDs and metadata separately from
fact identity, so the same fact can have multiple independent provenance
records:

#+begin_src lisp
(let ((fact-id (symbolic-fact-id '(grandparent "alice" "carol"))))
  (symbolic-explain-fact *db* fact-id))
#+end_src

This layer is intentionally reasoner-neutral. Tek9 owns durable symbolic state,
bounded matching, deterministic forward chaining, and provenance. A downstream
system such as llm-log or Prolog-RLM can materialize these records into
SWI-Prolog for richer expert reasoning while retaining Tek9 as the canonical
knowledge store.
